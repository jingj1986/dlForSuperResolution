...
local modelG = util.load('./model/mse_bgr_G_12', opt.gpu)
...
local fGx_mse = function(x)
    ...
    local errG = criterion_abs:forward(fake, real)
    local df_do_mse = criterion_abs:backward(fake, real)
...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      optim.adam(fGx_mse, parametersG, optimStateG)
   ...
   if epoch % 1 == 0 then
     util.save(opt.model_folder .. opt.name .. '_l1_adversarial_G_' .. epoch, modelG, opt.gpu)
   end
