...
--   local input_1 =  image.scale(input,opt.wild,opt.high,'bicubic')
    ...
    wild_s = 0
    high_s = 0
    local input_1 = image.crop(input,wild_s,high_s,opt.wild+wild_s,opt.high+high_s)
   
   --local input_y=image.rgb2y(input)
   --local input_y=image.rgb2y(input_tmp)
   
   local input_2=image.scale(input_1,opt.wild/opt.scale,opt.high/opt.scale,'bilinear')
   --local input_2=image.scale(input_1,opt.wild/opt.scale,opt.high/opt.scale,'bicubic')
