...
local fGx=function(x)
    ...
    modelG:backward(input,0.00001*df_dg+0.999*df_do_per)
    err_all=0.00001*errG+0.999*errG_per
...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % 10 == 0 then
          test:copy(real[1])
          local real_rgb=test
          image.save("./result_23/" .. opt.name..counter..'_real.png',real_rgb)
          test2:copy(input[1])
          image.save("./result_23/" .. opt.name..counter..'_input.png',test2)
          fake[fake:gt(1)]=1
          fake[fake:lt(0)]=0
          image.save("./result_23/" .. opt.name..counter..'_fake.png',fake[1])
      end
