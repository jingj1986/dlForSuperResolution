...
for epoch = 1, opt.niter do
   ...
   for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
      ...
      if counter % 200 == 0 then
          test:copy(real[1])
          local real_rgb=test
          image.save("./result_up/" .. opt.name..counter..'_real.png',deprocess(real_rgb))
          test2:copy(input[1])
          image.save("./result_up/" .. opt.name..counter..'_input.png',deprocess(test2))
          fake = deprocess(fake[1])
          fake[fake:gt(1)]=1
          fake[fake:lt(0)]=0
          image.save("./result_up/" .. opt.name..counter..'_fake.png',fake)
      end
   ...
   if epoch % 1 == 0 then
     util.save(opt.model_folder .. opt.name .. '_up_adversarial_G_' .. epoch, modelG, opt.gpu)
     util.save(opt.model_folder .. opt.name .. '_up_adversarial_D_' .. epoch, modelD, opt.gpu)
   end
