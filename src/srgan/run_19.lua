...
--local modelG = util.load('./model/shuff_G_12', opt.gpu)
local D=require 'adversarial_D.lua'
...
VGG19 = torch.load('models/VGG19.t7')
if opt.vggloss_type == '2,2' then
    for _ = 1, 27 do
        VGG19:remove()
    end
    --VGG19:remove(5)
elseif opt.vggloss_type == '5,4' then
    --VGG19:remove()
    print("use 5,4")
end
print(VGG19)
...
local fGx_only = function(x)
    ...
    fake = modelG:forward(input)
    local vgg_GT = VGG19:forward(real):clone()
    local vgg_fake = VGG19:forward(fake)
    errG = criterion_mse:forward(vgg_fake, vgg_GT)
    
    local df_mse = criterion_mse:backward(vgg_fake, vgg_GT)
    local df_vgg = VGG19:backward(fake, df_mse)
    modelG:backward(input, df_vgg)
    err_all = errG
