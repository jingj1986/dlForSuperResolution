...
class LabelTool():  
    def __init__(self, master):  
        ...
        self.lb1 = Label(self.frame, text = '舰船类型:') 
        self.lb1.grid(row = 1, column = 2,  sticky = W+N)
        self.list1 = Listbox(self.frame, width = 28, height = 10)  
        self.list1.grid(row = 2, column = 2, sticky = N)
        self.list1.insert(END, "merchant")
        self.list1.insert(END, "carrier")  
        self.list1.insert(END, "war")
        self.list1.insert(END, "relief")  
        self.list1.insert(END, "island") 
        self.list1.insert(END, "yacht") 
        self.list1.insert(END, "cage") 
        self.list1.insert(END, "submarine") 
        self.list1.insert(END, "other") 
        self.list1.selection_set(0,0)
...
##            return  
        ...
        if os.path.exists(self.labelfilename):  
            with open(self.labelfilename) as f:  
                for (i, line) in enumerate(f):  
                    ...
                    print DEST_SIZE  
                    #tmp = (0.1, 0.3, 0.5, 0.5)  
                    print "tmp[0,1,2,3,4]===%d, %d, %d, %d, %s" %(tmp[0], tmp[1], tmp[2], tmp[3], tmp[4])  
                    #print "%.2f,%.2f,%.2f,%.2f" %(tmp[0] tmp[1] tmp[2] tmp[3] )  
                    print "********************"  
                    #self.bboxList.append(tuple(tx))  
                    self.bboxList.append(tuple(tmp))  
                    tmpId = self.mainPanel.create_rectangle(tmp[0],tmp[1],tmp[2],tmp[3],\
    ...
    def mouseClick(self, event):  
        if self.STATE['click'] == 0:  
            ...
            ship_type = self.list1.get(self.list1.curselection()) if self.list1.curselection() else 'war'
             
            self.bboxList.append((x1, y1, x2, y2, ship_type))  
            self.bboxIdList.append(self.bboxId)  
            self.bboxId = None  
            self.listbox.insert(END, '(%d, %d)-(%d, %d): %s' %(x1, y1, x2, y2, ship_type))  
            self.listbox.itemconfig(len(self.bboxIdList) - 1, fg = COLORS[(len(self.bboxIdList) - 1) % len(COLORS)])  
