...
local modelG = util.load('./models/VDSR.t7', opt.gpu)
local D=require 'adversarial_D.lua'
local modelD = require('weight-init')(D(),'kaiming')
--local modelD = util.load('./model/vgg_mse_D_2', opt.gpu)
...
fake_per = torch.Tensor(opt.batchSize, 3, opt.loadSize, opt.loadSize)
real_per = torch.Tensor(opt.batchSize, 3, opt.loadSize, opt.loadSize)

local fGx=function(x)
    modelG:zeroGradParameters()
    label:fill(real_label)
    local output=modelD.output
    fake_per[{{},{1},{},{}}]:copy(fake)
    fake_per[{{},{2},{},{}}]:copy(fake)
    fake_per[{{},{3},{},{}}]:copy(fake)
    real_per[{{},{1},{},{}}]:copy(real)
    real_per[{{},{2},{},{}}]:copy(real)
    real_per[{{},{3},{},{}}]:copy(real)
    input=input:cuda()
    real=real:cuda()
    fake_per = preprocess['vgg'].preprocess(fake_per)
    real_per = preprocess['vgg'].preprocess(real_per)
    fake_per = fake_per:cuda()
    real_per = real_per:cuda()

    errG = criterion:forward(output, label)
    errG_per = criterion_percep:forward(fake_per,real_per)
    --errG_mse = criterion_mse:forward(fake, real)

    local df_do = criterion:backward(output, label)
    --local df_do_mse = criterion_mse:backward(fake, real)
    --local df_do_per = criterion_percep:backward(fake_per, real_per)[{{},{1},{},{}}]
    local df_do_per = criterion_percep:backward(fake_per, real_per)
    df_do_per[{{},{1},{},{}}] = (df_do_per[{{},{1},{},{}}] + df_do_per[{{},{2},{},{}}] + df_do_per[{{},{3},{},{}}]):div(255):div(3)

    local df_dg=modelD:updateGradInput(fake,df_do)
    print(errG)
    modelG:backward(input,0.001*df_dg+0.999*df_do_per[{{},{1},{},{}}])--+0.5*df_do_mse)
    --modelG:backward(input,0.999*df_do_per[{{},{1},{},{}}])--+0.5*df_do_mse)
    --modelG:backward(input,0.001*df_dg+0.999*df_do_mse)
    err_all=0.001*errG+0.999*errG_per -- +0.5*errG_mse
    --err_all=0.999*errG_per -- +0.5*errG_mse
...
for epoch = 1, opt.niter do
   ...
   if epoch % 1 == 0 then
     ...
     torch.save(opt.model_folder .. opt.name .. '_vdsr_' .. epoch, modelG:clone())
