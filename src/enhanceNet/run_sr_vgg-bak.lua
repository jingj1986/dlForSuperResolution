...
--local modelG = torch.load('./model/xjx2_vggmse_600_G.t7'):type(dtype)
print(modelG)
...
local vgg_weight=0.01
--local vgg_weight=0.1
...
--     util.save('./model/xjx2_vgg_' .. epoch .. '_D.t7', modelD, opt.gpu)
    torch.save('./model/xjx2_vggmse_' .. epoch .. '_G.t7', modelG)
    torch.save('./model/xjx2_vggmse_' .. epoch .. '_D.t7', modelD)
--        modelG:clearState()
--        modelG:float()
...
--        modelG:type(dtype)
   ...
   if epoch == 2000 then
        if use_cudnn then cudnn.convert(modelG, nn) end
        torch.save('./model/xjx2_vggmse_s_' .. epoch .. '_G.t7', modelG:clearState() )
        torch.save('./model/xjx2_vggmse_s_' .. epoch .. '_D.t7', modelD:clearState() )
        if use_cudnn then cudnn.convert(modelG, cudnn) end
  end
