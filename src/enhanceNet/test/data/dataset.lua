...
local function tableToOutput(self, dataTable, dataTable2, nameTable, scalarTable)
   local data, scalarLabels, labels, data2, name
   local quantity = #scalarTable
--   assert(dataTable[1]:dim() == 3)
   data = torch.Tensor(quantity,
		       --self.sampleSize[1], self.sampleSize[2], self.sampleSize[3])
                 opt.chan, opt.high, opt.wild)
   data2 = torch.Tensor(quantity,
           --self.sampleSize[1], self.sampleSize[2]/opt.scale, self.sampleSize[3]/opt.scale)
            opt.chan, opt.high/opt.scale, opt.wild/opt.scale)
   ...
   return data, data2, nameTable, scalarLabels
end
...
function dataset:sample(quantity)
   ...
   while i <= quantity do
      ...
      local out,out2, name = self:getByClass(class)
      if out then
   ...
   local data, data2, name, scalarLabels = tableToOutput(self, dataTable, dataTable2, nameTable, scalarTable)
   return data, data2, name, scalarLabels
end
...
function dataset:get(i1, i2)
   ...
   local data, scalarLabels = tableToOutput(self, dataTable, nil, scalarTable)
