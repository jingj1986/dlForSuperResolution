...
local function createModel()
      local function bottleneck()
          ...
          convs:add(nn.InstanceNormalization(64))
    ...
    local function multLayers()
        ...
        return nn.Sequential()
                :add(nn.ConcatTable()
                        ...
                        :add(nn.InstanceNormalization(64))
                        :add(nn.Identity()))
    ...
    main=nn.Sequential()
    main:add(nn.SpatialConvolution(3,64,3,3,1,1,1,1))
    --main:add(nn.SpatialConvolution(3,64,9,9,1,1,4,4))
    main:add(nn.InstanceNormalization(64))
    main:add(nn.ReLU())
    main:add(layer(16))
    ...
    main:add(nn.SpatialConvolution(64,3,3,3,1,1,1,1))
--[[
    main:add(nn.InstanceNormalization(3))
    main:add(nn.ReLU())
