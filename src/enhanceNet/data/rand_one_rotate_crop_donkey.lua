...
local function loadImage(path)
   ...
   if iW < opt.loadSize + 12 or iH < opt.loadSize + 12 then
   ...
   input = image.rotate(input, rotate_rand*math.pi/2)
   return input
end
...
local trainHook = function(self, path)
   ...
   local Xmin = math.floor(torch.uniform(6, w - opt.loadSize - 6))
   local Ymin = math.floor(torch.uniform(6, h - opt.loadSize - 6))
...
--   local input_HR = image.scale(input,opt.loadSize,opt.loadSize,'bilinear')
--   local input_LR = image.scale(input_HR,opt.loadSize/opt.scale,opt.loadSize/opt.scale,'bilinear')
   local input_LR = image.scale(input_HR,opt.loadSize/opt.scale,opt.loadSize/opt.scale,'bicubic')

--   k_size = math.floor(torch.uniform(2, 6))
--   ker = image.gaussian({size=k_size,normalize=true})
--   input_LR = image.convolve(input_LR, ker, 'same') 
   --local input_LR = image.scale(input_HR,opt.loadSize/4,opt.loadSize/4,'bicubic')
