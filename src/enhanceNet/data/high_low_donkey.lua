...
local trainHook = function(self, path)
   ...
   local h = low:size(2)
   local w = low:size(3)
   local Xmin = math.floor(torch.uniform(0, w - opt.loadSize/2))
   local Ymin = math.floor(torch.uniform(0, h - opt.loadSize/2))

   local Xmax = Xmin + opt.loadSize/2
   local Ymax = Ymin + opt.loadSize/2

   local input_LR = image.crop(low, Xmin, Ymin, Xmax , Ymax)  
   local input_HR = image.crop(high,Xmin*2,Ymin*2,Xmax*2,Ymax*2)
...
--[[
    ...
    img2 = torch.IntTensor(opt.loadSize/2, opt.loadSize/2)
    img2:copy(img1)
    img1:copy(img2)
    input_LR = img1:add(-16):mul(255):div(219):div(255)
]]--
